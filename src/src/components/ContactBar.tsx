...
const ContactBar: React.FC<ContactBarProps> = ({ visible = true }) => {
  ...
  return (
    <>
      {/* Mobile Version */}
      <div className={`fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-40 py-2 md:hidden transition-all duration-300 ${showMobileBar ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="w-full max-w-[1400px] mx-auto px-4">
          <div className="flex justify-around items-center">
            ...
            {/* WhatsApp */}
            <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center hover-lift">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#25d366] flex items-center justify-center mb-1 shadow-md">
                <svg className="text-white h-4 w-4 md:h-5 md:w-5" fill="currentColor" viewBox="0 0 24 24">
